...
- v23 adds a searchable all-entity browser for custom mob colors. The browser
  searches Russian names, English names, and registry IDs, and opens the
  existing RGBA/color-wheel editor for the selected entity.
- v23 adds separate searchable entity checklists for ALERT and RETURNED
  session membership. An empty checklist preserves the legacy behavior and
  allows every entity type; a new config starts with zombie, creeper, and
  skeleton selected.
- v23 adds client-side RETURNED detection: while an entity is absent, the
  player's maximum X/Z distance from its last position is tracked. On return,
  the entity is marked RETURNED when the configured distance is reached;
  `returnedDistanceBlocks` defaults to `86.0`.
...
If `alertMode` is `0`, a server whose MAX ID is below `alertGap` cannot produce
any ALERT entries. For example, MAX ID `19,599` with the default gap `100,000`
will correctly show an empty session; switch to percentage mode or lower the
gap in the ALERT settings.
