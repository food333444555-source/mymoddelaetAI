...
public final class EntityPickerScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int row = 0; row < rows; row++) {
            ...
            boolean checked = mode != Mode.COLORS && (selected.isEmpty() || selected.contains(entry.id));
        ...
        if (mode != Mode.COLORS) {
            counter += " | выбрано: " + (selected.isEmpty() ? "ВСЕ" : selected.size());
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (mode == Mode.COLORS) {
            ...
            if (selected.isEmpty()) selected.addAll(allEntityIds());
            if (!selected.remove(entry.id)) selected.add(entry.id);
            setSelection(selected.size() == allEntityIds().size() ? Set.of() : selected);
            save();
        }
        return true;
    }
