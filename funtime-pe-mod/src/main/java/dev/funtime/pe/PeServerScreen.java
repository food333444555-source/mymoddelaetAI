...
public final class PeServerScreen extends Screen {
    ...
    public PeServerScreen(Screen parent, PeServerEntry editing) {
        super(Text.literal(editing == null ? "Добавить сервер" : "Изменить сервер"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        this.protocol = field(left, 162, "Версия", Integer.toString(
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        final int left = this.width / 2 - 120;
        if (inside(mouseX, mouseY, left, 48, 240, 20) && name.mouseClicked(mouseX, mouseY, button)) {
            return true;
        }
        if (inside(mouseX, mouseY, left, 86, 240, 20) && host.mouseClicked(mouseX, mouseY, button)) {
            return true;
        }
        if (inside(mouseX, mouseY, left, 124, 240, 20) && port.mouseClicked(mouseX, mouseY, button)) {
            return true;
        }
        if (inside(mouseX, mouseY, left, 162, 240, 20) && protocol.mouseClicked(mouseX, mouseY, button)) {
            return true;
        }
        if (inside(mouseX, mouseY, left, 200, 240, 20)) {
            authMode = authMode == PeAuthMode.GUEST ? PeAuthMode.MICROSOFT : PeAuthMode.GUEST;
            this.clearAndInit();
            return true;
        }
        if (inside(mouseX, mouseY, left, 232, 115, 20)) {
            save();
            return true;
        }
        if (inside(mouseX, mouseY, left + 125, 232, 115, 20)) {
            close();
            return true;
        }
        if (editing != null && inside(mouseX, mouseY, left, 260, 240, 20)) {
            delete();
            return true;
        }
        return false;
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(this.textRenderer, Text.literal("Версия"), this.width / 2 - 120, 152, 0xA0A0A0);
