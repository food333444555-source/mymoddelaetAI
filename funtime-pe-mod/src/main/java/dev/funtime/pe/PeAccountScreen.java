...
public final class PeAccountScreen extends Screen {
    ...
    protected void init() {
        ...
        final ButtonWidget microsoftButton = ButtonWidget.builder(Text.literal(
                        microsoftLoginInProgress
                                ? "Microsoft: ожидание входа..."
    ...
    private void startMicrosoftLogin() {
        ...
        MicrosoftDeviceLogin.start(
                ...
                error -> client.execute(() -> {
                    microsoftLoginInProgress = false;
                    status = Text.literal("Ошибка Microsoft: " + safeMessage(error));
                    this.clearAndInit();
                }));
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        final int left = this.width / 2 - 120;
        if (inside(mouseX, mouseY, left, 68, 240, 20)
                && guestName.mouseClicked(mouseX, mouseY, button)) {
            return true;
        }
        if (inside(mouseX, mouseY, left, 100, 240, 20)) {
            saveGuest();
            return true;
        }
        if (inside(mouseX, mouseY, left, 132, 240, 20)) {
            startMicrosoftLogin();
            return true;
        }
        if (inside(mouseX, mouseY, left, 164, 240, 20)) {
            PeConfig.account().clearMicrosoftSession();
            PeConfig.save();
            status = Text.literal("Microsoft-профиль отключён.");
            this.clearAndInit();
        ...
        if (inside(mouseX, mouseY, left, 220, 240, 20)) {
            close();
            return true;
        }
        return false;
