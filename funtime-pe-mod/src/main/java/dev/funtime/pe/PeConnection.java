...
final class PeConnection {
    private static final AtomicLong CONNECTION_ATTEMPT = new AtomicLong();
    ...
    static void connect(MinecraftClient client, Screen parent, PeServerEntry server,
        ...
        final long attempt = CONNECTION_ATTEMPT.incrementAndGet();
        ...
        BedrockClient.connect(server, account,
                ...
                .whenComplete((connected, error) -> client.execute(() -> {
                    if (attempt != CONNECTION_ATTEMPT.get()) {
                        if (connected != null) {
                            connected.close();
                        }
                        return;
    ...
    static void closeActive() {
        CONNECTION_ATTEMPT.incrementAndGet();
        VanillaJavaBridgeRegistry.close();
        if (activeClient != null) {
            activeClient.close();
            activeClient = null;
